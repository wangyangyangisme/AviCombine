...
#ifdef WIN32
    AVI->fdes = _open(filename, O_RDWR|O_CREAT|O_BINARY, (S_IRUSR | S_IWUSR) &~ mask);
#else
...
#define AVIF_COPYRIGHTED        0x00020000
    ...
    if ( _lseek(AVI->fdes,0,SEEK_SET)<0 ||
        avi_write(AVI->fdes,(char *)AVI_header,HEADERBYTES)!=HEADERBYTES ||
        _lseek(AVI->fdes,AVI->pos,SEEK_SET)<0)
    {
...
int AVI_append_audio(avi_t *AVI, char *data, long bytes)
{
    ...
    //update chunk header
    _lseek(AVI->fdes, pos+4, SEEK_SET);
    long2str(c, length+bytes);     
    avi_write(AVI->fdes, (char *)c, 4);

    _lseek(AVI->fdes, pos+8+length, SEEK_SET);
...
#ifdef WIN32  
    AVI->fdes = _open(filename,O_RDONLY|O_BINARY);
#else
...
int avi_parse_input_file(avi_t *AVI, int getIndex)
{
    ...
    while(1)
    {
        ...
        if(strncasecmp(data,"LIST",4) == 0)
        {
            ...
            if(strncasecmp(data,"hdrl",4) == 0)
            {
                ...
                header_offset = _lseek(AVI->fdes,0,SEEK_CUR);
    ...
    if(!hdrl_data      ) ERR_EXIT(AVI_ERR_NO_HDRL)
        if(!AVI->movi_start) ERR_EXIT(AVI_ERR_NO_MOVI)
                        ...
                        if (wfe != NULL) {
                            ...
                            if (wfe->cb_size != 0) {
                                nwfe = (char *)realloc(wfe, sizeof(WAVEFORMATEX_avilib) +
                                    wfe->cb_size);
                                if (nwfe != 0) {
                                    off_t lpos = _lseek(AVI->fdes, 0, SEEK_CUR);
                                    _lseek(AVI->fdes, header_offset + i + sizeof(WAVEFORMATEX_avilib),
                                        SEEK_SET);
                                    wfe = (WAVEFORMATEX_avilib *)nwfe;
                                    nwfe = &nwfe[sizeof(WAVEFORMATEX_avilib)];
                                    avi_read(AVI->fdes, nwfe, wfe->cb_size);
                                    _lseek(AVI->fdes, lpos, SEEK_SET);
                                }
                            }
            ...
            _lseek(AVI->fdes,AVI->movi_start,SEEK_SET);
            ...
            if(AVI->idx)
            {
                ...
                if(avi_read(AVI->fdes,data,8)!=8) ERR_EXIT(AVI_ERR_READ)
                    ...
                    {
                        _lseek(AVI->fdes,pos+AVI->movi_start-4,SEEK_SET);
                        if(avi_read(AVI->fdes,data,8)!=8) ERR_EXIT(AVI_ERR_READ)
                            if( strncasecmp(data,(char *)AVI->idx[i],4)==0 && str2ulong((unsigned char *)data+4)==len )
                            {
            ...
            if(idx_type == 0)
            {
                ...
                while(1)
                {
                    ...
                    if( ( (data[2]=='d' || data[2]=='D') &&
                        ...
                        (data[3]=='b' || data[3]=='B') ) )
                    {
                        avi_add_index_entry(AVI,(unsigned char *)data,0,_lseek(AVI->fdes,0,SEEK_CUR)-8,n);
                    }

                    _lseek(AVI->fdes,PAD_EVEN(n),SEEK_CUR);
                }
                idx_type = 1;
            }
...
long AVI_read_frame(avi_t *AVI, char *vidbuf, int *keyframe)
{
    ...
    *keyframe = (AVI->video_index[AVI->video_pos].key==0x10) ? 1:0;
    ...
    _lseek(AVI->fdes, AVI->video_index[AVI->video_pos].pos, SEEK_SET);
    ...
    while(1)
    {
        ...
        else if(strncasecmp(data,AVI->track[AVI->aptr].audio_tag,4) == 0)
        {
            *len = n;
            ...
            if(_lseek(AVI->fdes,n,SEEK_CUR)<0)  return 0;
    }
}
